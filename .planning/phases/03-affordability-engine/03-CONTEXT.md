...
- **D-17 (resolves RESEARCH Open Question 1 — baseline savings; user-confirmed 2026-06-26):** The household block gains an explicit **`currentAnnualSavings`** field (what the household saves per year *today, while renting* — canonical decimal string, annual). The **savings-rate ceiling** is then well-defined: `postPurchaseAnnualSavings(price) = currentAnnualSavings − annualOwnershipPremium`, where the ownership premium is the **incremental cash drain** `(monthly cash savings drain − currentRent) × 12` using the D-14 savings-drain numerator (`tco.total − amortizedClosing`, which INCLUDES maintenance). The max affordable price is the largest price where `postPurchaseAnnualSavings(price) / grossIncome ≥ targetSavingsRate` (D-04). Rationale: the researcher's fallback (assume baseline == `targetSavingsRate × grossIncome`) is degenerate — any premium > $0 fails the floor — so a real, independent baseline input is required. `currentAnnualSavings` is the single number the household already knows; the living-expenses-derivation alternative was rejected to keep the contract minimal. This field is part of the durable `household`/profile shape (D-09) Phase 6 will persist.
