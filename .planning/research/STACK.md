...
**Domain:** Personal financial decision tool (home-affordability + FI-impact engine) — Next.js + TypeScript shell over a pure, framework-agnostic calculation core, local SQLite persistence, adapter-isolated listings.
**Researched:** 2026-06-22
**Confidence:** HIGH on core framework/testing/persistence/money-math (verified against current npm `latest` tags and official docs); MEDIUM on charting (choice is a judgment call, not a correctness issue).
...
```bash
# --- root (monorepo) ---
npm init -y                       # set "private": true, add "workspaces": ["packages/*","apps/*"]
npm install -D typescript@6 vitest@4 @vitest/coverage-v8 prettier eslint

# --- packages/core (pure calc engine) ---
npm install decimal.js@10 -w packages/core
npm install -D vitest@4 -w packages/core
# NO react, NO next, NO dom libs here — that's the point

# --- apps/web (Next.js shell) ---
npm install next@16 react@19 react-dom@19 -w apps/web
npm install drizzle-orm@0 better-sqlite3@12 zod@4 zustand@5 recharts@3 -w apps/web
npm install "@boston-fi/core@*" -w apps/web    # workspace dep on the core package
npm install -D drizzle-kit@0 @types/better-sqlite3 -w apps/web
```
...
---
*Stack research for: personal home-affordability + FI-impact financial engine (Next.js + pure TS core + local SQLite)*
*Researched: 2026-06-22*
